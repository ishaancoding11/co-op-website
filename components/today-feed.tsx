...
import {
  categoryLabel, displayNameFor, priceRange,
  type CreativeCategory, type Currency,
} from '@/lib/types';
...
type CreativeRow = {
  ...
  rate_min: number | null; rate_max: number | null; currency: Currency | null;
...
type JobRow = {
  ...
  budget_min: number | null; budget_max: number | null; currency: Currency | null;
  created_at: string;
    ...
    <div className="py-8 space-y-9">
      ...
      {activeRole === 'creative' && freshJobs.length > 0 && (
        <Section
          ...
          hint={creativeCategories.length ? `Matching ${creativeCategories.map(c => categoryLabel(c, locale)).join(', ')}` : undefined}
          href="/jobs"
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-2 text-xs text-muted">
                      {priceRange(j.budget_min, j.budget_max, j.currency ?? undefined) && (
                        <span className="font-semibold text-foreground">{priceRange(j.budget_min, j.budget_max, j.currency ?? undefined)}</span>
                      )}
                      <span>·</span>
        ...
        <Section
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            {nearbyCreatives.map(c => {
              ...
              const rate = priceRange(c.rate_min, c.rate_max, c.currency ?? undefined);
              return (
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex flex-wrap gap-1 mt-2">
                      {(c.categories ?? []).slice(0, 2).map(x => (
                        <Tag key={x} tone="accent">{categoryLabel(x, locale)}</Tag>
                      ))}
                    </div>
                  </div>
                </Link>
