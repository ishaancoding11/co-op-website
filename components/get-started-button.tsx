...
export function GetStartedButton({ className = '', children = 'Get started' }: { className?: string; children?: React.ReactNode }) {
  return (
    <button onClick={() => window.dispatchEvent(new Event(OPEN_GATE_EVENT))}
      className={className || 'rounded-full bg-foreground text-background px-4 py-2 text-sm font-medium active:scale-[0.97] hover:opacity-90 transition-[transform,opacity] duration-200 ease-[var(--ease-out)]'}>
      {children}
    </button>
  );
}
