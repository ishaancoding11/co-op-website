...
import { Card, Avatar, EmptyState, Tag, StatusBadge, IconHeart, LinkButton } from '@/components/ui';
      ...
      {!favs?.length ? (
        <EmptyState
          ...
          body={activeRole === 'business'
            ? 'Spotted a creative you might book later? Tap the heart on their profile to keep them here.'
            : 'See a job you want to circle back to? Tap the heart on it and it will wait for you here.'}
          action={<LinkButton href={activeRole === 'business' ? '/browse' : '/jobs'} variant="secondary">
            {activeRole === 'business' ? 'Browse creatives' : 'Browse jobs'}
          </LinkButton>} />
      ) : (
        <>
          {creatives.length > 0 && <>
              ...
              {creatives.map(f => {
                ...
                return (
                  <Link key={f.id} href={`/creatives/${f.saved_creative_id}`} className="block">
                    <Card className="p-4 flex items-center gap-4 hover:border-line-strong hover:shadow-[var(--shadow-md)] transition-[border-color,box-shadow] duration-200 ease-[var(--ease-out)]">
              ...
              {jobs.map(f => {
                ...
                return (
                  <Link key={f.id} href={`/jobs/${f.saved_job_id}`} className="block">
                    <Card className="p-4 flex items-center justify-between gap-3 hover:border-line-strong hover:shadow-[var(--shadow-md)] transition-[border-color,box-shadow] duration-200 ease-[var(--ease-out)]">
                      <div>
