...
export default async function Favorites() {
  ...
  const { data: favs } = await supabase.from('favorites')
    .select('*, creative_profiles(neighborhood, categories, avatar_url, rate_min, rate_max, users(display_name)), jobs(title, category, status, budget_min, budget_max)')
      ...
      {!favs?.length ? (
        ...
        <>
                  ...
                  <Link key={f.id} href={`/creatives/${f.saved_creative_id}`} className="block">
                    <Card className="p-4 flex items-center gap-4 hover:shadow-[0_8px_30px_rgba(45,42,38,0.1)] transition-shadow">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted">{cp?.neighborhood} · {priceRange(cp?.rate_min ?? null, cp?.rate_max ?? null) ?? ''}</p>
                      </div>
          ...
          {jobs.length > 0 && <>
            ...
            <div className="space-y-3">
              {jobs.map(f => {
                const j = f.jobs as { title: string; category: CreativeCategory; status: string; budget_min: number | null; budget_max: number | null } | null;
                return (
                  <Link key={f.id} href={`/jobs/${f.saved_job_id}`} className="block">
                    <Card className="p-4 flex items-center justify-between gap-3 hover:shadow-[0_8px_30px_rgba(45,42,38,0.1)] transition-shadow">
                      <div>
                        ...
                        <p className="text-xs text-muted">{j ? CATEGORY_LABELS[j.category] : ''} · {priceRange(j?.budget_min ?? null, j?.budget_max ?? null) ?? 'Budget TBD'}</p>
                      </div>
