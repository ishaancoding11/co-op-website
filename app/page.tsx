  ...
  return (
    <div className="py-14 md:py-24">
      ...
      <div className="max-w-4xl">
        ...
        <p className="animate-rise text-accent font-medium mt-5 text-sm" style={{ animationDelay: '140ms' }}>
          Subscribe now — you won&rsquo;t be charged until your first match.
        </p>
        <p className="animate-rise text-sm text-muted mt-2" style={{ animationDelay: '160ms' }}>
        ...
        <div className="mt-5 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
          {ALL_CATEGORIES.map(c => (
            <Link key={c} href={`/browse?category=${c}`}
              ...
              <span className="text-sm font-semibold leading-tight group-hover:text-accent transition-colors">{CATEGORY_LABELS[c] ?? c}</span>
            </Link>
          ))}
        </div>
      </div>
      ...
      <div className="mt-20 border-t border-line-strong pt-8 grid grid-cols-2 sm:grid-cols-4 gap-y-6 gap-x-4 max-w-2xl">
        <div>
        ...
        <div className="sm:pl-6 sm:border-l border-line">
        ...
        <div className="sm:pl-6 sm:border-l border-line">
          <p className="font-display text-4xl text-accent">$0</p>
          <p className="text-muted text-xs mt-1.5 tracking-wide">until your first match</p>
        </div>
      </div>
    </div>
  );
