...
## Iteration 5 — admin panel (2026-08-01)
Adapted from the same teammate repo, admin/support code this time (`0010_support_admin.sql`, `admin-actions.ts`, `admin-ui.tsx`, the `/admin` route tree):
- **Taken**: the `notFound()`-not-redirect gate on `/admin` (a non-staff visitor gets an indistinguishable 404, not a "no permission" tell), the `is_staff()`/`is_admin()` security-definer pattern re-derived from `auth.uid()` on every call — never trusted from an argument — the moderator/admin two-tier split, "suspension requires a typed reason, is always logged, never deletes," and the "no client JS required" philosophy for the admin forms (plain server actions, no `useActionState`).
- **Discarded**: his `message_flags`/`conversations.is_flagged` content-scanning system (we have no message-scanning feature at all — flagged-conversations was dropped from scope rather than faked), his fee/GMV/escrow numbers in `admin_overview()`, his `profiles.role` enum reuse (ours is a separate `staff_role` column so it can't collide with the existing creative/business role-lock logic), and his support-ticket system (not requested).
- **Staff identity**: `users.staff_role` (`moderator`|`admin`, nullable = not staff) has zero client-facing write path — no signup flow, no RPC, no RLS insert/update policy touches it. The only way to grant it is a direct SQL `UPDATE` in the Supabase dashboard. Went one step further than the teammate's version: `staff_role` and the new `status` column are **column-level REVOKEd** from `authenticated`/`anon` entirely (the existing `users_select` policy is `using (true)` for public display names, which would otherwise leak staff_role/status to any client via a plain `select *`). The only sanctioned read is `my_staff_role()` (security-definer, your own row only).
- **Suspension enforcement**: goes further than the teammate's version, which only pulled open jobs off the feed. Ours also blocks new job posts, new agreement creation, and new messages via `is_account_active()` checks added into the existing quota-guard triggers (`jobs_quota_guard`, `agreements_quota_guard`) plus a new one for `messages` (which had no guard trigger at all before). Existing matches/messages/agreements stay fully readable — nothing is hidden or deleted retroactively.
- **Job removal** is `moderator`+ (content moderation); **account suspension** stays `admin`-only, per confirmation.
- Reports table existed since the MVP as pure data-capture (insert-only, no read policy at all) — this iteration is the first time anything can read or resolve them (`admin_reports`/`admin_resolve_report`, both staff-gated).
- New `audit_log` table (didn't exist before): append-only, staff-read-only via RLS, written exclusively by the `admin_*` security-definer functions.
